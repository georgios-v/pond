...
VOID_TASK_PROTO_5(pairalign_task, int, m, int, n, int, si, int, sj, int, len1);
TASK_PROTO_4(int, pairalign, int, istart, int, iend, int, jstart, int, jend);
...
//void pairalign_task (  int m, int n, int si, int sj, int len1) //First-private passed as argument

VOID_TASK_5(pairalign_task, int, m, int, n, int, si, int, sj, int, len1) {
	//Private data
	int i, len2;
	double mm_score, gg;
...
TASK_4(int, pairalign, int, istart, int, iend, int, jstart, int, jend) {
	int i, n, m, si, sj;
	int len1, len2, maxres;
	double gg, mm_score;
	int *mat_xref, *matptr;
	int tot_task = 0; //Task_Sync count...
		...
		if ((n = seqlen_array[si + 1]) != 0) {
			...
			for (sj = si + 1; sj < nseqs; sj++) {
				if ((m = seqlen_array[sj + 1]) != 0) {
					SPAWN(pairalign_task, m, n, si, sj, len1);
					tot_task++;
				}
			}
		}
	...
	//printf("Tot_Tasks: %d\n",tot_task);
	for (i = 0; i < tot_task; i++) SYNC(pairalign_task);
...
void align() {
	//	vfs_remove(NON_EXISTENT_FILE);
	// printf("In align now\n");
	ROOT_CALL(pairalign, 0, nseqs, 0, nseqs);
}
...
int alignment(APP_ARGS) {
	...
	print("\n %d -- %s %s \n", argc, argv[0], argv[1]);
	char alignment_file[15848] = ALIGNMENT_DATA;
	bots_check_flag = BOTS_CONFIG;
	bots_sequential_flag = BOTS_SEQUENTIAL;

	pairalign_init(alignment_file);
	bots_set_info("Protein alignment (Single version)", false);
