...
void allocate_village(struct Village **capital, struct Village *back,
	...
	if (level == 0) *capital = NULL;
		...
		*capital = (struct Village *) malloc(sizeof (struct Village));
		...
		(*capital)->hosp.realloc_lock.locked = false
		(*capital)->hosp.realloc_lock.queue = NULL
		(*capital)->hosp.realloc_lock
		(*capital)->hosp.realloc_lock = PTHREAD_MUTEX_INITIALIZER;
		if (pthread_mutex_init(&((*capital)->hosp.realloc_lock), NULL) != 0) printf("Error mutex\n");
		...
		if (p->time_left == 0) {
			...
			if (rand < sim_convalescence_p) {
				...
				if (rand > sim_realloc_p || village->level == sim_level) {
					...
					pthread_mutex_lock(&(village->hosp.realloc_lock));
					addList(&(village->back->hosp.realloc), p);
					pthread_mutex_unlock(&(village->hosp.realloc_lock));
...
VOID_TASK_1(sim_village_par, struct Village*, village) {
	struct Village *vlist;
	int ntask = 0;
	int i;
	...
	if ((sim_level - village->level) < bots_cutoff_value) {
		while (vlist) {
			SPAWN(sim_village_par, vlist);
			ntask++;
			vlist = vlist->next;
		}
	} else {
		while (vlist) {
			CALL(sim_village_par, vlist);
			vlist = vlist->next;
		}
	}
...
VOID_TASK_1(sim_village_start, struct Village*, top) {
	long i;
	for (i = 0; i < sim_time; i++) CALL(sim_village_par, top);
}

void sim_village_main_par(struct Village *top) {
	ROOT_CALL(sim_village_start, top);
}
