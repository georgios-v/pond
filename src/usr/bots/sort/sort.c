...
VOID_TASK_PROTO_5(cilkmerge_par, ELM*, low1, ELM*, high1, ELM*, low2, ELM*, high2, ELM*, lowdest);
VOID_TASK_PROTO_3(cilksort_par, ELM*, low, ELM*, _tmp, long, size);
 ...
 */
static void seqquick(ELM *low, ELM *high) {
	ELM *p;
...
static void seqmerge(ELM *low1, ELM *high1, ELM *low2, ELM *high2,
		ELM *lowdest) {
	ELM a1, a2;
...
static ELM *binsplit(ELM val, ELM *low, ELM *high) {
	/*
...
VOID_TASK_5(cilkmerge_par, ELM*, low1, ELM*, high1, ELM*, low2, ELM*, high2, ELM*, lowdest) {
	/*
	...
	*(lowdest + lowsize + 1) = *split1;
	...
	SPAWN(cilkmerge_par, low1, split1 - 1, low2, split2, lowdest);

	CALL(cilkmerge_par, split1 + 1, high1, split2 + 1, high2,
			lowdest + lowsize + 2);
...
VOID_TASK_3(cilksort_par, ELM*, low, ELM*, _tmp, long, size) {
	/*
	...
	SPAWN(cilksort_par, A, tmpA, quarter);
	//#pragma omp task untied
	SPAWN(cilksort_par, B, tmpB, quarter);
	//#pragma omp task untied
	SPAWN(cilksort_par, C, tmpC, quarter);
	//#pragma omp task untied
	CALL(cilksort_par, D, tmpD, size - 3 * quarter);
	...
	//#pragma omp task untied
	SPAWN(cilkmerge_par, A, A + quarter - 1, B, B + quarter - 1, tmpA);
	//#pragma omp task untied
	CALL(cilkmerge_par, C, C + quarter - 1, D, low + size - 1, tmpC);

	SYNC(cilkmerge_par);
	//#pragma omp taskwait
	//     cilk_sync;

	CALL(cilkmerge_par, tmpA, tmpC - 1, tmpC, tmpA + size - 1, A);
}

static void scramble_array(ELM *_array) {
	unsigned long i;
	unsigned long j;
...
static void fill_array(ELM *_array) {
	unsigned long i;
...
void sort_init(void) {
	/* Checking arguments */
	if (bots_arg_size < 4) {
		bots_message("Array size can not be less than 4, using 4 as a parameter.\n");
		bots_arg_size = 4;
	}

	if (bots_app_cutoff_value < 2) {
		bots_message("Sequential Merge cutoff value can not be less than 2, using 2 as a parameter.\n");
		bots_app_cutoff_value = 2;
	} else if (bots_app_cutoff_value > bots_arg_size) {
		bots_message("Sequential Merge cutoff value can not be greather than vector size, using %d as a parameter.\n", bots_arg_size);
		bots_app_cutoff_value = bots_arg_size;
	}

	if (bots_app_cutoff_value_1 > bots_arg_size) {
		bots_message("Sequential Quicksort cutoff value can not be greather than vector size, using %d as a parameter.\n", bots_arg_size);
		bots_app_cutoff_value_1 = bots_arg_size;
	}
	if (bots_app_cutoff_value_2 > bots_arg_size) {
		bots_message("Sequential Insertion cutoff value can not be greather than vector size, using %d as a parameter.\n", bots_arg_size);
		bots_app_cutoff_value_2 = bots_arg_size;
	}

	if (bots_app_cutoff_value_2 > bots_app_cutoff_value_1) {
		bots_message("Sequential Insertion cutoff value can not be greather than Sequential Quicksort cutoff value, using %d as a parameter.\n",
...
void sort_par(opt_worker_t self) {
	bots_message("Computing multisort algorithm (n=%d) ", bots_arg_size);
	//	#pragma omp parallel
	//	#pragma omp single nowait
	//	#pragma omp task untied
	ROOT_CALL(cilksort_par, array, tmp, bots_arg_size);
