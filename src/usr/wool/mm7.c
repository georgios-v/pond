...
LOOP_BODY_PROTO_4(mm7, LARGE_BODY / 1, int, i, int, rows, double*, a, double*, b, double*, c);

LOOP_BODY_4(mm7, LARGE_BODY / 1, int, i, int, rows, double*, a, double*, b, double*, c) {
	int j;
...
int mm7(APP_ARGS) {
	...
	for (i = 0; i < reps; i++) {
		double *t;
		ROOT_FOR(mm7, 0, rows, rows, a, b, c);
		t = a;
		a = c;
		c = t;
	}
